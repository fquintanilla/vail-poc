...
- Keep the main component entrypoint server-first. When interactivity is needed, push only the interactive slice into a dedicated client file instead of turning the whole component into `"use client"`.
...
3. **Use modern Next.js primitives appropriately** — keep `index.tsx` as a Server Component by default, use `next/image` for images, and keep client boundaries narrow when needed.
